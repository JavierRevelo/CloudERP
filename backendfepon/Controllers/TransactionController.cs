    ...
    public class TransactionController : BaseController
    {
        ...
        public async Task<ActionResult<TransactionDTO>> PostTransaction(CreateUpdateTransactionDTO transactionDTO)
        {
            ...
            catch (DbUpdateException ex)
            {
                ...
                return StatusCode(500, GenerateErrorResponse(500, "Error al actualizar la base de datos, no es posible crear la Asociación1", ex));
            }
            catch (AutoMapperMappingException ex)
            {
                // Handle AutoMapper exceptions
                return StatusCode(500, GenerateErrorResponse(500, "Error en la configuración del mapeo, no es posible crear la Asociación2", ex));
            }
            catch (Exception ex)
            {
